package flashx.textLayout.compose
{
   ...
   public class StandardFlowComposer extends FlowComposerBase implements IFlowComposer
   {
      ...
      public function updateToController(param1:int = 2.147483647E9) : Boolean
      {
         ...
         var _loc4_:Boolean = this.areShapesDamaged();
         if(_loc4_)
         {
      ...
      private function releaseLines(param1:ContainerController) : void
      {
         ...
         var _loc4_:int = lines.length;
         var _loc5_:int = !!param1?int(findLineIndexAtPosition(param1.absoluteStart)):0;
         while(_loc5_ < _loc4_)
         {
            _loc6_ = lines[_loc5_];
            ...
            if(_loc7_ != _loc2_)
            {
               if(!_loc3_ && _loc2_)
               {
                  _loc2_.releaseTextBlock();
               }
               _loc2_ = _loc7_;
               _loc3_ = false;
            }
            if(!_loc3_ && !_loc6_.isDamaged())
            {
               _loc8_ = _loc6_.peekTextLine();
